 ...
 * @param raw 后端 JSON
 */
export function parseBackendConversation(raw) {
    if (!raw || !raw.mapping) {
        throw new Error("无效的 conversation 数据：缺少 mapping 字段");
    }
    /** 逐条解析节点 */
    const mapping = {};
    for (const [id, nodeRaw] of Object.entries(raw.mapping)) {
        mapping[id] = {
            id,
 ...
 * 按时间升序展开整棵树
 */
export function flattenConversation(conv) {
...
function parseMessage(raw) {
    ...
    return {
        ...
        end_turn: typeof raw.end_turn === "boolean" ? raw.end_turn : Boolean(raw.end_turn),
        weight: raw.weight ?? 0,
